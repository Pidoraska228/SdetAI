...
// =============================================================================
void SparseDynamicNetwork::train_step(float input_token, float target_token, float learning_rate) {
    // 1. Inject input token
    inject_input(&input_token, 1);

    // 2. Forward pass through all groups (one cycle)
    run_cycle(1);

    // 3. Read network prediction (output of last group)
    ...
    // 5. Update trainable parameters: projection_matrix and sparse weights
    for (auto& group : groups_) {
        // Update projection matrix (dense, small)
        for (size_t i = 0; i < group.projection_matrix.size(); ++i) {
            // Simple gradient step proportional to error
            group.projection_matrix[i] += learning_rate * error * 0.01f;
        }

        // Update sparse connection weights
        for (size_t w = 0; w < group.weights.size(); ++w) {
            // Hebbian-like / gradient step
            group.weights[w] += learning_rate * error * 0.001f;
        }
    }
}
