...
void Trainer::train_on_tokens(
    ...
    // БЫЛО: 0.0005f — работало со старым OUTPUT_CLAMP=50, но после
    // того как clamp поднят до реального диапазона id токенов, тот же
    // learning_rate давал дикие скачки loss (до 361M против обычных
    // ~1M за счёт значительно возросшего диапазона возможных значений).
    // Понижение в 10 раз убрало скачки на тесте.
